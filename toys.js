...
(function () {
  ...
  var TOYS = {
    ...
    pingala:       { step: LENIA, creature: { w: 20, h: 20, cells: ORBIUM }, slots: 2, sameWay: true, mode: 0, size: [112, 63], sps: 1, every: 1, life: 1600, ink: PLUM, ink2: PLUM2, u: { u_R: 13, u_T: 10, u_mu: 0.15, u_sigma: 0.015, u_nb: 1, u_beta: [1, 0, 0, 0] } },
  ...
  function start(canvas, name) {
    ...
    function reseed() {
      ...
      if (seedP) { bindTarget(1 - cur); run(seedP, cfg.u); cur = 1 - cur; }
        ...
        var slots = cfg.slots || 3, slotW = Math.floor(W / slots);
        var sameX = Math.random() < 0.5, sameY = Math.random() < 0.5;
        for (var n = 0; n < slots; n++) {
          var ox = n * slotW + 3 + Math.floor(Math.random() * Math.max(1, slotW - cw - 6));
          var oy = 3 + Math.floor(Math.random() * Math.max(1, H - ch - 6));
